...
import { applyGuytonKlinger, type DrawdownGuardrailPolicy, type GuardrailState } from "../drawdown/guytonKlinger.js";
...
export interface PersonYearResult {
  ...
  /** Set only when the caller supplied `runProjection`'s `drawdownGuardrails` param and a Guyton-Klinger source drove this person's drawdown this year — `"none"` when guardrails were active but neither rule triggered, `undefined` (not `"none"`) whenever guardrails weren't requested at all. */
  readonly guardrailAdjustment?: "cut" | "raise" | "none";
 ...
 *
 * `drawdownGuardrails`, likewise omitted by every caller except the
 * Confidence page's opt-in Guyton-Klinger toggle, substitutes a
 * dynamically-adjusted raw target for a `targetDrawdownIncome` source's
 * own static `targetNetAnnualIncome` at the exact point it's about to be
 * netted against automatic income (`adjustDrawdownTargetForAutomaticIncome`)
 * — see `drawdown/guytonKlinger.ts`. Omitted, this function's drawdown
...
export function runProjection(
  ...
  // Guyton-Klinger dynamic withdrawal guardrails (SPEC.md's Confidence
  // page) — each targetDrawdownIncome source's own baseline rate and
  // currently-carried target, keyed by that source's own id so separate
  // phases (and a joint vs per-person target) never share state. Declared
  // but never read or written when `drawdownGuardrails` is omitted — zero
  // effect on this function's default behaviour.
  let guardrailStateBySourceId = new Map<string, GuardrailState>();
  ...
  for (let yearIndex = 0; yearIndex < numberOfYears; yearIndex++) {
    ...
    const nextGuardrailStateBySourceId = new Map(guardrailStateBySourceId);
    ...
    // Every currently-active target instance's own effective figure
    // (guardrail-nudged when `drawdownGuardrails` is active, otherwise its
    // own stated `targetNetAnnualIncome` unchanged), summed per person it
    // applies to (almost always just one instance) — the cap Pass 2b uses
    // to decide how much achieved income counts as automatically spent,
    // not the *adjusted* (already-netted) target used to size the
    // withdrawal itself.
    const personTargetTotal = new Map<PersonId, Pence>();
    // Guyton-Klinger's cut/raise event for whichever source drove each
    // person's drawdown this year, surfaced on `PersonYearResult.guardrailAdjustment`
    // — only ever written when `drawdownGuardrails` was supplied (see
    // `resolveEffectiveTarget` below), so this stays empty and every
    // lookup below falls through to `undefined` otherwise.
    const guardrailEventByPersonId = new Map<PersonId, "cut" | "raise" | "none">();

    /**
     * Resolves one active `targetDrawdownIncome` source's raw target for
     * this year: its own static `targetNetAnnualIncome`, completely
     * unchanged, whenever `drawdownGuardrails` wasn't supplied (this is
     * the guarantee that omitting it reproduces today's behaviour
     * byte-for-byte) — or the Guyton-Klinger-nudged figure when it was.
     * `drawdownPoolValue` must be this source's own pooled
     * drawdown-eligible balance (pension+ISA+cash+GIA), the same
     * denominator `solveDrawdown`/`solveHouseholdDrawdown` themselves draw
     * from below, never net worth.
     */
    const resolveEffectiveTarget = (
      source: { readonly id: string },
      config: TargetDrawdownIncomeConfig,
      applicablePersonIds: readonly PersonId[],
      drawdownPoolValue: Pence,
      otherNetIncome: Pence,
    ): Pence => {
      if (!drawdownGuardrails) return config.targetNetAnnualIncome;
      const guardrailResult = applyGuytonKlinger(
        guardrailStateBySourceId.get(source.id),
        config,
        drawdownPoolValue,
        otherNetIncome,
        drawdownGuardrails,
      );
      nextGuardrailStateBySourceId.set(source.id, guardrailResult.nextState);
      for (const personId of applicablePersonIds) guardrailEventByPersonId.set(personId, guardrailResult.event);
      return guardrailResult.rawTarget;
    };
    ...
    for (const source of scenario.incomeSources) {
      ...
      if (source.owner === "joint") {
        ...
        const balancesByPerson = new Map(
          eligiblePeople.map((p) => [p.pass1.person.id, balancesFor(accountIdsByPerson.get(p.pass1.person.id) ?? discoverAccountIds(p.pass1.person.id))]),
        );
        const householdPeopleInputs = eligiblePeople.map((p) => {
          const balances = balancesByPerson.get(p.pass1.person.id) ?? balancesFor(discoverAccountIds(p.pass1.person.id));
        ...
        const applicablePersonIds = eligiblePeople.map((p) => p.pass1.person.id);
        const householdDrawdownPoolValue = sumPence(
          [...balancesByPerson.values()].flatMap((b) => [b.pensionBalance, b.isaBalance, b.cashBalance, b.giaBalance]),
        );
        const effectiveTarget = resolveEffectiveTarget(source, config, applicablePersonIds, householdDrawdownPoolValue, householdOtherNetIncome);
        for (const personId of applicablePersonIds) {
          personTargetTotal.set(personId, addPence(personTargetTotal.get(personId) ?? zeroPence(), effectiveTarget));
        }
        const adjustedHouseholdTarget = adjustDrawdownTargetForAutomaticIncome(effectiveTarget, householdOtherNetIncome);
      ...
      const otherNetIncome = otherNetIncomeByPersonId.get(source.owner) ?? zeroPence();
      const drawdownPoolValue = sumPence([balances.pensionBalance, balances.isaBalance, balances.cashBalance, balances.giaBalance]);
      const effectiveTarget = resolveEffectiveTarget(source, config, [source.owner], drawdownPoolValue, otherNetIncome);
      personTargetTotal.set(source.owner, addPence(personTargetTotal.get(source.owner) ?? zeroPence(), effectiveTarget));
      const adjustedTarget = adjustDrawdownTargetForAutomaticIncome(effectiveTarget, otherNetIncome);
    ...
    const perPerson: PersonYearResult[] = pass2aResults.map((pass2a) => {
      ...
      const guardrailAdjustment = guardrailEventByPersonId.get(person.id);
      ...
      return {
        ...
        dividendTax,
        drawdownShortfall,
        ...(guardrailAdjustment !== undefined ? { guardrailAdjustment } : {}),
        drawdownBuckets,
    ...
    guardrailStateBySourceId = nextGuardrailStateBySourceId;
    rows.push({
      taxYear,
      calendarYear,
      perPerson,
